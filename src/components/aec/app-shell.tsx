...
import { Bell, Boxes, BriefcaseBusiness, ChevronDown, CircleDollarSign, ClipboardCheck, Command, ContactRound, FileCheck2, GitBranch, LayoutDashboard, Menu, PackageSearch, Search, Settings2, ShoppingCart, UsersRound, X } from "lucide-react";
...
export function AppShell({ children, title, eyebrow, action }: { children: ReactNode; title: string; eyebrow?: string; action?: ReactNode }) {
  ...
  return <div className="app-frame">
    ...
    {open && <div className="mobile-drawer"><div className="flex items-center justify-between"><Brand /><Button variant="ghost" size="icon" aria-label="Fechar menu" onClick={() => setOpen(false)}><X /></Button></div><nav>{nav}</nav></div>}
